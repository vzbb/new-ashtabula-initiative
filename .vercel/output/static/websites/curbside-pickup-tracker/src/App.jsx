...
function App() {
  ...
  const check = async () => {
    ...
    setStatus("");
    try {
      const prompt = `Write a short curbside pickup status message for order code ${code}. Include pickup instructions. 60 words max.`;
  ...
  return (
    <div className="page">
      ...
      <header className="hero glass">
        <div>
          <p className="eyebrow">Curbside Pickup Tracker</p>
          <h1>Reduce wait time with instant pickup updates</h1>
          <p className="sub">Gemini-powered status messaging.</p>
          <div className="hero-actions">
            <button className="primary" onClick={check} disabled={loading || !code}>
              {loading ? "Checking…" : "Check Status"}
            </button>
            <button className="ghost">Enable SMS</button>
          </div>
          <div className="trust">
            <span><CarIcon/> Gemini API</span>
            <span><ZapIcon/> 5-second output</span>
            <span><CheckIcon/> Fewer calls</span>
          </div>
        </div>
        <div className="hero-card glass-card">
          <h3>Order</h3>
          <label>Order code<input value={code} onChange={(e) => setCode(e.target.value)} /></label>
        </div>
      </header>
      ...
      <section className="grid">
        <div className="tile glass-card"><h3>Faster pickups</h3><p>Clear instructions reduce delays.</p></div>
        <div className="tile glass-card"><h3>Better experience</h3><p>Customers know exactly when to arrive.</p></div>
        <div className="tile glass-card"><h3>Less admin</h3><p>Auto-responses cut inbound calls.</p></div>
      </section>

      <footer className="footer">Local Retail • Ashtabula County, OH</footer>
    </div>
  );
}
