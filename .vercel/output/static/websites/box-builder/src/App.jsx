...
function App() {
  const [spec, setSpec] = useState("12x10x8, 500 qty");
  ...
  const quote = async () => {
    ...
    setSummary("");
    try {
      const prompt = `Write a concise box build quote summary for ${spec}. Include price range and lead time. 60 words max.`;
  ...
  return (
    <div className="page">
      ...
      <header className="hero glass">
        <div>
          <p className="eyebrow">Box Builder</p>
          <h1>Instant custom box quotes</h1>
          <p className="sub">Gemini-powered summary + lead time.</p>
          <div className="hero-actions">
            <button className="primary" onClick={quote} disabled={loading}>
              {loading ? "Quoting…" : "Generate Quote"}
            </button>
            <button className="ghost">Request samples</button>
          </div>
          <div className="trust">
            <span><BoxIcon/> Gemini API</span>
            <span><ZapIcon/> 5-second output</span>
            <span><CheckIcon/> Faster close</span>
          </div>
        </div>
        <div className="hero-card glass-card">
          <h3>Specs</h3>
          <label>Details<input value={spec} onChange={(e) => setSpec(e.target.value)} /></label>
        </div>
      </header>
      
      <section className="card glass">
        <div className="card-head"><h2>Quote Summary</h2><span className="pill">Gemini API</span></div>
        {error && <div className="error">{error}</div>}
        {summary ? <pre className="output">{summary}</pre> : <p className="muted">Summary appears here.</p>}
      </section>
      
      <section className="grid">
        <div className="tile glass-card"><h3>Faster response</h3><p>Instant ranges.</p></div>
        <div className="tile glass-card"><h3>Lead time clarity</h3><p>Set expectations.</p></div>
        <div className="tile glass-card"><h3>Upsell options</h3><p>Premium finishes.</p></div>
      </section>
      
      <footer className="footer">Northeast Box Co. • Ashtabula County, OH</footer>
    </div>
  );
}
