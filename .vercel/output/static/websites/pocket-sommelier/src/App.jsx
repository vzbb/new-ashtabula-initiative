...
function App() {
  ...
  const recommend = async () => {
    ...
    setPairing("");
    try {
      const prompt = `Recommend a wine pairing for ${dish}. Include a premium bottle suggestion. 60 words max.`;
  ...
  return (
    <div className="page">
      <header className="hero">
        <div>
          <p className="eyebrow">Pocket Sommelier</p>
          <h1>Instant wine pairings for every table</h1>
          <p className="sub">Gemini‑powered recommendations + upsell.</p>
          <div className="hero-actions">
            <button className="primary" onClick={recommend} disabled={loading}>
              {loading ? "Pairing…" : "Get Pairing"}
            </button>
            <button className="ghost">Enable tableside</button>
          </div>
          <div className="trust">
            <span>🍷 Gemini API</span>
            <span>⚡ 5‑second output</span>
            <span>✅ Higher ticket</span>
          </div>
        </div>
        <div className="hero-card">
          <h3>Dish</h3>
          <label>Dish<input value={dish} onChange={(e) => setDish(e.target.value)} /></label>
        </div>
      </header>
      <section className="card">
        <div className="card-head"><h2>Recommendation</h2><span className="pill">Gemini API</span></div>
        {error && <div className="error">{error}</div>}
        {pairing ? <pre className="output">{pairing}</pre> : <p className="muted">Recommendation appears here.</p>}
      </section>
      <section className="grid">
        <div className="tile"><h3>Higher check size</h3><p>Premium bottle suggestions.</p></div>
        <div className="tile"><h3>Staff assist</h3><p>Instant guidance for servers.</p></div>
        <div className="tile"><h3>Branded voice</h3><p>Consistent recommendations.</p></div>
      </section>
      <footer className="footer">Ferrante Winery • Ashtabula County, OH</footer>
    </div>
  );
}
