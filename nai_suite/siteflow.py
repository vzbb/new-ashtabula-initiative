...
def get_vite_base(site_dir: Path) -> str | None:
    """Extract the base path from vite.config.js/ts"""
    import re
    for config_name in ["vite.config.js", "vite.config.ts"]:
        config_path = site_dir / config_name
        if config_path.exists():
            text = config_path.read_text()
            # Match base: '/something/' or base: "/something/"
...
def check_vite_base_matches_route(slug: str, site_dir: Path) -> tuple[bool, str, str]:
    ...
    """
    # For Vercel route-prefixed static sites, two base styles are valid:
    # - "./" (recommended): emits relative asset URLs that work under any prefix
    # - "/<slug>/": emits absolute URLs scoped to the route prefix
    expected_route_base = f"/{slug}/"
    expected_base = f"./ or {expected_route_base}"
    current_base = get_vite_base(site_dir)
    
    if current_base is None:
        # If base is missing, Vite defaults to "/", which typically breaks assets
        # under a route prefix like "/<slug>/".
        return False, "(missing)", expected_base
    
    if current_base in {"./", expected_route_base}:
        return True, current_base, expected_base
    
    return False, current_base, expected_base
