...
function App() {
  const [query, setQuery] = useState("Alternator for 2015 F‑150");
  ...
  const confirm = async () => {
    ...
    setReply("");
    try {
      const prompt = `Write a concise parts request confirmation for ${query}. Include next step to confirm fitment. 60 words max.`;
  ...
  return (
    <div className="page">
      <header className="hero">
        <div>
          <p className="eyebrow">Parts Finder Request</p>
          <h1>Confirm parts faster</h1>
          <p className="sub">Gemini‑powered request confirmations.</p>
          <div className="hero-actions">
            <button className="primary" onClick={confirm} disabled={loading}>
              {loading ? "Confirming…" : "Generate Confirmation"}
            </button>
            <button className="ghost">Reserve part</button>
          </div>
          <div className="trust">
      ...
      <section className="card">
        <div className="card-head"><h2>Confirmation</h2><span className="pill">Gemini API</span></div>
        {error && <div className="error">{error}</div>}
        {reply ? <pre className="output">{reply}</pre> : <p className="muted">Confirmation appears here.</p>}
      </section>
      <section className="grid">
        <div className="tile"><h3>Faster close</h3><p>Instant confirmations.</p></div>
        <div className="tile"><h3>Less admin</h3><p>Automated messaging.</p></div>
        <div className="tile"><h3>Better service</h3><p>Clear next steps.</p></div>
      </section>
      <footer className="footer">Ace Parts Company • Ashtabula County, OH</footer>
    </div>
  );
}
