...
// URL mapping for sites with different routes
const URL_MAP = {};

const BASE_URL = "https://new-ashtabula-initiative.vercel.app";
const OUTPUT_DIR = process.argv[2] || './screenshots-kimi-35-51';
...
(async () => {
  console.log(`\n========================================`);
  console.log(`  KIMI VISUAL QA — SITES 35-51`);
  console.log(`  AUTHENTIC BRANDING FOCUS`);
  ...
  for (let i = 0; i < SITES.length; i++) {
    ...
    console.log(`\n[${siteNum}/51] KIMI ANALYZING: ${site}`);
    console.log(`URL: ${url}`);
    ...
    try {
      ...
      if (desktopStats.size < THRESHOLDS.desktop) {
        brandingIssues.push("Desktop page too small - possible blank/minimal content");
      }
      if (tabletStats.size < THRESHOLDS.tablet) {
        brandingIssues.push("Tablet page too small - possible blank/minimal content");
      }
      if (mobileStats.size < THRESHOLDS.mobile) {
        brandingIssues.push("Mobile page too small - possible blank/minimal content");
      }
      ...
      console.log(`${statusEmoji} ${site}: ${result.status}`);
  ...
  // Summary Report
  console.log(`\n\n========================================`);
  console.log(`  KIMI QA SUMMARY — SITES 35-51`);
  console.log(`========================================\n`);
  ...
  // Write JSON report
  const reportPath = join(OUTPUT_DIR, 'kimi-35-51-report.json');
  ...
  const fixList = results
    ...
    .map(r => r.site);
  const fixPath = join(OUTPUT_DIR, 'kimi-fix-list.txt');
