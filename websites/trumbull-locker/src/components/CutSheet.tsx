...
export default function CutSheet({ animalType, onBack, onAddToCart, cartItems }: CutSheetProps) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="mb-12">
        <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full bg-[#e7efe6] text-[#254336] text-xs font-semibold uppercase tracking-[0.28em] mb-6">
          Trumbull Meat Locker
        </div>
        <h1 className="text-4xl md:text-6xl font-serif text-[#24382f] mb-4">{animal.name} Cut Sheet</h1>
        <p className="text-xl text-stone-600 max-w-3xl">
          Select the specific cuts you want from the diagram below. Customize your order by choosing the exact weight in pounds for your Trumbull Meat Locker cut sheet.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 flex flex-col gap-8">
          ...
          <div>
            <h3 className="text-xl font-serif text-[#24382f] mb-4">All Available Cuts</h3>
                ...
                return (
                  <button
                    ...
                    className={`
                      ...
                      ${isSelected 
                        ? 'border-[#254336] bg-[#eef4ee] shadow-md transform scale-[1.02]' 
                        : 'border-stone-200 bg-white text-stone-700 hover:border-[#9ab09a] hover:shadow-sm'
                      }
                    `}
                  >
                    {isSelected && (
                    ...
                    <div className="relative z-10 flex items-center justify-between w-full">
                      <span className={`flex items-center ${isSelected ? 'text-amber-900' : ''}`}>
                        {isSelected && <Check className="w-4 h-4 mr-1.5 text-[#254336]" />}
                        {cut.name}
                      </span>
                      {inCartQty > 0 && (
                        <span className={`ml-2 px-2 py-0.5 rounded-full text-xs font-bold ${isSelected ? 'bg-[#254336] text-amber-100' : 'bg-emerald-100 text-emerald-700'}`}>
                          {inCartQty} lbs
                        </span>
                      )}
                    </div>
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-24 bg-stone-50 rounded-3xl p-8 border border-[#d8d0c2] shadow-sm">
            {selectedCut ? (
              <motion.div
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-3xl font-serif text-[#24382f]">{selectedCut.name}</h3>
                  {selectedCut.popular && (
                    <span className="inline-block px-2 py-1 bg-[#e7efe6] text-[#254336] text-xs font-bold rounded-full uppercase tracking-wider">
                      Popular
                    </span>
                  )}
                </div>
                ...
                <div className="mb-8">
                  ...
                  <div className="flex items-center justify-between bg-white border border-stone-200 rounded-xl p-2">
                    <button 
                      ...
                      className="w-12 h-12 flex items-center justify-center text-stone-500 hover:bg-[#e7efe6] rounded-lg transition-colors"
                    >
                ...
                <button
                  onClick={handleAdd}
                  className="w-full bg-[#254336] hover:bg-[#1c3328] text-white py-4 rounded-xl font-medium text-lg flex items-center justify-center transition-colors shadow-md"
                >
