/**
 * API Configuration for MyTrip Planner Export
 ...
 * @param {string} theme - Trip theme (e.g., "Family", "Romantic", "Adventure")
 * @param {string} days - Number of days for the trip
 * @returns {Promise<string>} Generated itinerary text
 */
export async function generateItinerary(theme, days) {
  // Validate inputs
  if (!theme?.trim()) {
    throw new Error('Please provide a trip theme.');
  }
  ...
  const prompt = `Create a ${days}-day Ashtabula County, OH itinerary for a ${theme} trip. Return a simple day-by-day list, 120 words max.`;
