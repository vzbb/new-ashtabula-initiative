...
export async function digestConversation(messages = []) {
  if (!RAG_API_URL) throw new Error("RAG API URL not configured.");

  const response = await fetch(RAG_API_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json", Accept: "application/json" },
    body: JSON.stringify({
      question: "PLEASE SUMMARY AND EXTRACT: Based on the full conversation, please provide a JSON object with two fields: 'residentSummary' (a 1-2 sentence summary of what the resident is trying to do) and 'aiContextSummary' (a 1-2 sentence summary of the zoning advice/clerk's findings provided so far). Return ONLY valid JSON.",
  ...
  if (!response.ok) throw new Error("Digestion service unavailable.");
  const data = await response.json();
  ...
  try {
    ...
    return {
      residentSummary: messages.filter(m => m.role === 'user').pop()?.content || "",
      aiContextSummary: messages.filter(m => m.role === 'assistant').pop()?.content || ""
    };
  }
}
...
function buildRequestPayload(draft) {
  return {
    ...
    sourceCitations: typeof draft.sourceCitations === 'string' 
      ? draft.sourceCitations 
      : Array.isArray(draft.sourceCitations)
        ? draft.sourceCitations
        : Array.isArray(draft.citations)
          ? draft.citations
          : [],
    chatTranscript: typeof draft.chatTranscript === 'string'
      ? draft.chatTranscript
      : Array.isArray(draft.chatTranscript) 
        ? draft.chatTranscript 
        : [],
