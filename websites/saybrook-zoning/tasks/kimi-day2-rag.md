# Saybrook Zoning Clerk MVP - Day 2: RAG Query API
...
3. **prompts/** - Prompt templates
   - saybrook-zoning_expert.txt - System prompt for saybrook-zoning Q&A
   - permit_advisor.txt - System prompt for permit guidance

## RAG Pipeline:
1. Receive query (e.g., "Can I build a garage at 123 Main St?")
2. Extract address → lookup saybrook-zoning district (mock for now)
3. Search Qdrant for relevant code sections
4. Assemble context: saybrook-zoning district + relevant sections
5. Format prompt with context + query
6. Return: {answer, sources, confidence, follow_up_questions}

## Mock data for MVP:
Create saybrook-zoning_districts.json with sample Ashtabula districts:
- R-1 Residential
- R-2 Residential
- C-1 Commercial
- C-2 Commercial
