# Saybrook Zoning Clerk — Phase 3B Implementation Checklist
## Advanced Features & Pre-Deployment Polish

**Project:** Saybrook Zoning Clerk (RAG-based saybrook-zoning chatbot)  
...
def ingest_with_metadata(file_path: str, collection_name: str = "saybrook-zoning_docs"):
        ...
        for i, para in enumerate(paragraphs):
            ...
            points.append(PointStruct(
                ...
                payload={
                    ...
                    "document_type": "saybrook-zoning_code" if "saybrook-zoning" in filename.lower() else "historic"
                }
            ))
...
export async function POST(req: NextRequest) {
  try {
    ...
    const searchResult = await qdrant.search('saybrook-zoning_docs', {
      vector: queryVector,
      limit: 5,
      with_payload: true,
    });
    ...
    const systemPrompt = `You are the Ashtabula Saybrook Zoning Clerk Assistant. Answer saybrook-zoning questions using ONLY the provided context.
...
5. For saybrook-zoning decisions, always mention that official confirmation from the City Zoning Office is recommended
...
export function ChatPanel() {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content: 'Hello! I\'m the Ashtabula Saybrook Zoning Clerk Assistant. Ask me about saybrook-zoning districts, setbacks, permit requirements, or historic district guidelines.',
    },
  ]);
      ...
      <div className="p-4 border-t bg-white">
        <div className="flex gap-2">
          <input
            ...
            placeholder="Ask about saybrook-zoning, setbacks, permits..."
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      ...
      <header className="bg-blue-900 text-white">
        <div className="max-w-6xl mx-auto px-4 py-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold">Ashtabula Saybrook Zoning Clerk</h1>
              <p className="text-blue-200 text-sm">AI-powered saybrook-zoning assistance</p>
            </div>
      ...
      <footer className="max-w-6xl mx-auto px-4 py-6 text-center text-sm text-slate-500">
        <p>© 2025 Ashtabula Saybrook Zoning Clerk Assistant — Built for the City of Ashtabula, Ohio</p>
...
curl -X POST http://localhost:3000/api/chat \
  ...
  -d '{"message": "What are the fence height limits in R-1 saybrook-zoning?"}'
...
pm2 start npm --name "saybrook-zoning" -- start
pm2 save
```
