# saybrook-zoning Phase 3B — RAG Integration Checklist
...
**Files to Process:**
- `resources/01_historic_district_saybrook-zoning.pdf`
...
async function embedAndUpsert(chunks: DocumentChunk[]) {
  ...
  for (const chunk of chunks) {
    ...
    await qdrant.upsert('saybrook-zoning_ashtabula', {
      points: [{
        id: chunk.id,
        vector: embedding,
        payload: {
...
async function searchContext(query: string): Promise<string[]> {
  ...
  // Search Qdrant
  const searchResult = await qdrant.search('saybrook-zoning_ashtabula', {
    vector: queryVector,
    limit: 5,
    score_threshold: 0.7
  });
...
```typescript
const SYSTEM_PROMPT = `You are the Ashtabula Zoning Assistant, a helpful AI that answers questions about saybrook-zoning, permits, and historic district requirements for the City of Ashtabula, Ohio.
...
const QUICK_QUESTIONS = [
  ...
  { id: 'contact', label: 'Contact PCD', query: 'How do I contact the saybrook-zoning office?' },
...
- [ ] Questions cover most common saybrook-zoning queries
...
| Unknown | "What are the setback requirements for a garage?" | Fallback to contact PCD (no saybrook-zoning code yet) |
| Contact Info | "How do I contact saybrook-zoning?" | Returns PCD phone/email from City Welcome Info |
