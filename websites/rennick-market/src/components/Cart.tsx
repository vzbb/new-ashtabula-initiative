...
export default function Cart({ items, onRemove, onBack, onCheckout }: CartProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <div className="flex items-center justify-between mb-12 border-b border-[#e6d8c6] pb-8">
        <h1 className="text-4xl md:text-5xl font-serif text-[#4b1a1a] flex items-center">
          <Receipt className="w-10 h-10 mr-4 text-[#6a2b2b]" />
          Rennick Order Sheet
        </h1>
      ...
      <p className="mb-8 max-w-2xl text-sm leading-7 text-stone-600">
        This quote-style order sheet gives Rennick's team a fast read on requested cuts and expected weight. Final pricing and pickup timing can still be confirmed by the counter.
      </p>
      ...
      {items.length === 0 ? (
        <div className="text-center py-24 bg-stone-50 rounded-3xl border border-stone-200">
          ...
          <p className="text-stone-500 mb-8 max-w-md mx-auto">Looks like you haven't selected any cuts yet. Head back to Rennick Meat Market to start building your order.</p>
          <button
            onClick={onBack}
            className="bg-[#6a2b2b] hover:bg-[#4b1a1a] text-white px-8 py-3 rounded-full font-medium transition-colors"
          >
            Browse Market Cuts
          </button>
        </div>
        ...
        <div className="space-y-12">
          {Object.entries(groupedItems).map(([animalType, animalItems]) => {
            ...
            return (
              <div key={animalType} className="bg-white rounded-3xl p-8 shadow-sm border border-[#e6d8c6]">
                <div className="flex items-center justify-between mb-6 border-b border-stone-100 pb-4">
                  <h2 className="text-2xl font-serif text-[#4b1a1a] capitalize flex items-center">
                    {animal.name} Cuts
                  </h2>
                ...
                <div className="space-y-4">
                  {animalItems.map((item) => (
                    <motion.div
                      ...
                      key={item.id}
                      className="flex items-center justify-between p-4 bg-stone-50 rounded-2xl border border-stone-100 group hover:border-[#d7bb7a] transition-colors"
                    >
        ...
        <div className="flex justify-end pt-8">
          <button
            onClick={onCheckout}
            className="bg-[#6a2b2b] hover:bg-[#4b1a1a] text-white px-12 py-4 rounded-2xl font-medium text-lg shadow-lg transition-transform hover:-translate-y-1 active:translate-y-0"
          >
            Submit Order to Rennick
          </button>
        </div>
      </div>
      )}
