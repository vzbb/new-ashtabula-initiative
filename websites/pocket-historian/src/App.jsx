...
function App() {
  const [topic, setTopic] = useState("Shipwrecks");
  ...
  const generate = async () => {
    ...
    setStory("");
    try {
      const prompt = `Write a 100-word local history snippet about ${topic} in Ashtabula County. End with a CTA to visit the museum.`;
  ...
  return (
    <div className="page">
      <header className="hero">
        <div>
          <p className="eyebrow">Pocket Historian</p>
          <h1>Local history in seconds</h1>
          <p className="sub">Gemini‑powered storytelling for visitors.</p>
          <div className="hero-actions">
            <button className="primary" onClick={generate} disabled={loading}>
              {loading ? "Generating…" : "Generate Story"}
            </button>
            <button className="ghost">Enable audio tour</button>
          </div>
          <div className="trust">
      ...
      <section className="card">
        <div className="card-head"><h2>Story</h2><span className="pill">Gemini API</span></div>
        {error && <div className="error">{error}</div>}
        {story ? <pre className="output">{story}</pre> : <p className="muted">Story appears here.</p>}
      </section>
      <section className="grid">
        <div className="tile"><h3>Visitor engagement</h3><p>Short, shareable stories.</p></div>
        <div className="tile"><h3>Easy updates</h3><p>New topics anytime.</p></div>
        <div className="tile"><h3>Tour ready</h3><p>Use as exhibit copy.</p></div>
      </section>
